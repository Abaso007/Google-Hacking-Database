...
def append_sitename(strs,site):
    ...
    strs = [f'{x} site:{site}' for x in strs]
    return strs

def gen_google_query(strs):
    """generating Google query strings for each line"""
    qs = [f"http://www.google.com/search?q={quote_plus(x)}" for x in strs]
    return [strs,qs]
...
def gen_html_output(strs,q):
    ...
    res = [
        '<html>\n',
        '<head><title>SecPoint.com GoogleDB queries strings</title></head>\n',
        '<body>\n',
        '<p>Generated by: <a href="http://www.secpoint.com/">SecPoint.com</a> GoogleDB tool</p>\n',
        '\t<ul>\n',
    ]
    res.extend('\t\t<li><a href="%s">%s</a></li>\n'%(v,x) for x, v in zip(strs,q))
    res.extend(('\t</ul>\n', '</body>\n</html>'))
    return res
